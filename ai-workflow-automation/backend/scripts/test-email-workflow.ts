import { nodeRegistry } from '../src/services/nodeRegistry';
...
async function testEmailWorkflow() {
    ...
    const discoveryNode = nodeRegistry.get('EMAIL_DISCOVERY');
    if (!discoveryNode) {
    ...
    if (emails.length === 0) {
      ...
      `
        ...
        const sendingNode = nodeRegistry.get('EMAIL_SENDING');
        if (!sendingNode) {
